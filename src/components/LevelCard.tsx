...
export function LevelCard({ number, completed, best, onPress, disabled }: {
  number: number; completed: boolean; best?: number; onPress: () => void; disabled?: boolean;
}) {
  const { t } = useTranslation();
  return <View style={styles.card}>
    <AppText variant="subtitle">{t('level', { number })}</AppText>
    <AppText variant="muted">{t(completed ? 'completed' : 'available')}</AppText>
