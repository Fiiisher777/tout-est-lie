import { playtestEnabled } from '../config/environment';
...
export function placeholderAds(labels: { title: string; body: string; reward: string; cancel: string }): AdsService {
  return { showInterstitial: async () => 'unavailable', showRewarded: () => !playtestEnabled ? Promise.resolve('unavailable') : new Promise(resolve => {
