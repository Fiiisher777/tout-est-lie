...
test.each([[1, 120], [2, 150], [3, 180], [4, 210], [5, 240]] as const)('difficulty %i uses configured %i seconds', (difficulty, seconds) => { expect(countdownDuration(difficulty, 4)).toBe(seconds * 1000); });
test('background and ad pauses overlap without counting suspended time', () => {
  ...
  expect(remainingTime(s, 100000)).toBe(110000); s = setPaused(s, 'ad', false, 100000); expect(remainingTime(s, 101000)).toBe(109000);
});
test('timeout freezes and is not yet a terminal failure', () => {
  const s = checkCountdown(fresh(), clock(120000)); expect(s.timedOut).toBe(true); expect(s.status).toBe('playing'); expect(s.activeSince).toBeNull();
...
test('one extension preserves layout, mistakes, solved groups and session', () => {
  ...
  const timed = checkCountdown(s, clock(120000)); const continued = continueCountdown(timed, clock(999999));
...
test('second timeout fails with unchanged mistakes and frozen active time', () => {
  const s = continueCountdown(checkCountdown(fresh(), clock(120000)), clock(200000));
  const failed = checkCountdown(s, clock(230000)); expect(failed.status).toBe('lost'); expect(failed.mistakes).toBe(0);
  expect(produceCompletionResult(failed)).toMatchObject({ outcome: 'lost', failureReason: 'timeout', elapsedMs: 150000 });
  expect(continueCountdown(failed, clock(300000))).toBe(failed);
});
test('declining timeout ends the attempt exactly once', () => { const failed = endTimeout(checkCountdown(fresh(), clock(120000)), clock(130000)); expect(failed.status).toBe('lost'); expect(endTimeout(failed, clock(200000))).toBe(failed); });
test('restart cannot restore mistakes, time, hints, or rewarded eligibility', () => {
  ...
  s = continueCountdown(checkCountdown(s, clock(120000)), clock(200000)); const restarted = restartAttempt(s);
...
test('restart cannot dismiss timeout or final failure', () => { const s = checkCountdown(fresh(), clock(120000)); expect(restartAttempt(s)).toBe(s); });
