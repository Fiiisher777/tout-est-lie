...
test('version three round trips', () => { const s = defaultPlayer(); s.completedLevels = ['en-easy']; s.highestUnlockedLevel.en = 2; expect(migratePlayer(JSON.stringify(s)).state).toEqual(s); });
...
test('future versions are protected', () => { expect(migratePlayer('{"version":4}').writable).toBe(false); });
